...
def RNNModel_predictor(data, number_of_step_ahead, RNN_Type, Monthly_seasonality, Weekly_seasonality, frequency):
    ...
    if (Weekly_seasonality and frequency == "Daily"):
        my_model.fit(series, future_covariates=covariates_week.astype(np.float32))
    elif (Monthly_seasonality):
        my_model.fit(series, future_covariates=covariates_month.astype(np.float32))
    else:
        my_model.fit(series)
