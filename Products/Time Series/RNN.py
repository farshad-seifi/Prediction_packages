...
def RNNModel_predictor(data, number_of_step_ahead,RNN_Type):
    ...
    if len(data) >= 21 and frequency != 'None':
        ...
        for i in range(0, len(data["date"]) + number_of_step_ahead):
            if (frequency == "Monthly"):
                ...
                data["date"].iloc[len(data["date"]) - i - 1] = str(year) + "-" + str(month) + "-" + str(
                    day) + " " + str(hour) + ":00:00"
                date[0].iloc[len(date[0]) - i - 1] = str(year) + "-" + str(month) + "-" + str(day) + " " + str(
                    hour) + ":00:00"
...
results = RNNModel_predictor(train_data, number_of_step_ahead, RNN_Type)
